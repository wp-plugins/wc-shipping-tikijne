<?php
	...
	function plugin_register(){
	    ...
	    if (extension_loaded('curl')) {
                  $ch = curl_init();
                  curl_setopt($ch, CURLOPT_URL, "http://www.epeken.com/plugin_register/?emailid=".$current_user->user_email."&website=".$url."&plugin=wc_shipping_tikijne&ver=1.2.5.1-free");
                  curl_setopt($ch, CURLOPT_HEADER, 0);
                  curl_exec($ch);
                  curl_close($ch);    
             }	
	}
	add_action('wp_ajax_wc_shipping_tikijne_plugin_register','plugin_register');
